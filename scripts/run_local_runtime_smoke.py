...
def run_ts_smoke(repo_root: Path, runtime_profile_file: Path, profile_name: str, mission_id: str, objective: str):
    ...
    try:
        command = ["npx", "--yes", "tsx", str(temp_path)]
...
def main():
    ...
    completed = run_ts_smoke(repo_root, runtime_profile_file, args.profile, args.mission_id, args.objective)
    errors = []
    payload = {}
    ...
    verdict = "pass" if completed.returncode == 0 and lifecycle.get("status") == "terminal" and not errors else "fail"
    reason_code = "ok" if verdict == "pass" else "local_runtime_smoke_failed"
    ...
    report = {
        ...
        "ts_command": ["npx", "--yes", "tsx", "<tempfile>"],
